...
상태: false
유형: 명칭 사전
상세: Nuclei 분석에서 사용하는 프로그램 구조·CLI·DSL·보안 명칭의 공통 정의
tags:
  - 전지성
  - nuclei/명칭사전
  - 명칭
...
서로 수학적으로 연결된 개인키와 공개키의 한 쌍이다. 개인키는 서명을 만들고 공개키는 해당 서명을 검증한다.

### 인증서
#명칭/보안/인증서

공개키와 사용자·조직 같은 식별 정보를 묶고 전자서명한 데이터다. Nuclei 서명 기능에서는 공개키와 `Subject.CommonName`을 가진 X.509 인증서를 만든다.

### 자체 서명 인증서
#명칭/보안/자체-서명-인증서

별도의 인증기관이 아니라 인증서에 들어 있는 공개키와 대응하는 개인키로 스스로 서명한 인증서다. 암호학적으로 내용 변경을 검증할 수 있지만, 입력한 사용자 이름의 신원을 외부 기관이 보증한다는 뜻은 아니다.

### Passphrase 또는 암호 구문
#명칭/보안/passphrase #명칭/보안/암호-구문

저장된 개인키를 암호화하거나 다시 열 때 사용하는 비교적 긴 비밀 문자열이다. 로그인 비밀번호와 목적은 비슷하지만 여기서는 개인키 파일 보호에 사용한다.

### ECDSA
#명칭/보안/ecdsa

타원곡선 암호를 이용하는 전자서명 알고리즘이다. Nuclei의 사용자 키 생성 코드는 P-256 곡선으로 키 쌍을 만들고 SHA-256과 함께 인증서 서명에 사용한다.

### P-256
#명칭/보안/p-256

ECDSA에서 사용할 수 있는 표준 타원곡선 중 하나다. 키 생성 코드의 `elliptic.P256()`이 이 곡선을 선택한다.

### PEM
#명칭/보안/pem

인증서나 암호 키의 이진 데이터를 Base64 문자와 시작·끝 표시로 감싸 파일에 저장하는 형식이다. `nuclei-user-private-key.pem`이 이 형식을 사용한다.

### Common Name
#명칭/보안/common-name

X.509 인증서의 Subject에 포함되는 대표 식별자 필드다. `-sign` 최초 키 생성 과정에서 입력한 사용자·조직 이름이 이 필드에 저장된다.
